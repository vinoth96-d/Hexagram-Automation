...
public class MM_Full_BackDate_NoChangeINT extends BasePage{

    
	String ExcelFileName = BasePage.readURLTestDataProp("MoneyMarket_TestData");
	
	String Scenario = "MC BackDate(T+3) No change INT";
	
	@Test(priority = 1)
	public void Create_DepositPlacement() throws Exception
	{
		test=extent.createTest("Create Deposit Placement on BackDate");
	//	test.log(Status.INFO, "MultiCurrency - Full Withdrawal(Back Date,No Change in Interest)");
	...
	{
		test=extent.createTest("Create Bulk Deposit Maturity(FullWithdrawal,No Change in INT,On Maturity)");
		 

		BulkDepositMaturityPage.BulkDepositMaturity_Fullwithdrawal(ExcelFileName, Scenario,"","MultiCurrency");
