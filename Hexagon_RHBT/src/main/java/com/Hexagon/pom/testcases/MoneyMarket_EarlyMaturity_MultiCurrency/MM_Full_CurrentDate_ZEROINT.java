...
public class MM_Full_CurrentDate_ZEROINT extends BasePage{

    
	String ExcelFileName = BasePage.readURLTestDataProp("MoneyMarket_TestData");
	String Scenario = "MC CurrentDate(T+2) Zero INT";
	...
	public void Create_DepositPlacement() throws Exception
	{
		...
		test.log(Status.INFO, "MultiCurrency -Early Maturity(2 days) FullWithdrawal(Current Date,Zero Interest)");
	...
	{
		test=extent.createTest("Create Bulk Deposit Maturity(FullWithdrawal,Change in INT)");
		 
		
		BulkDepositMaturityPage.BulkDeposit_EarlyMaturity_Fullwithdrawal(ExcelFileName, Scenario,"INT","MultiCurrency");
	...
	{
		test=extent.createTest("Verify Bulk Deposit Maturity in Accounting Enquiry Report");
	...
	{
		test=extent.createTest("Verify Bulk Deposit Maturity in Bank Transactions Report");
