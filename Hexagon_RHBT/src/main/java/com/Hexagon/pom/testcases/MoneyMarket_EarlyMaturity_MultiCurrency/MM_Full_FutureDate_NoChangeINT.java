...
public class MM_Full_FutureDate_NoChangeINT extends BasePage{

    
	String ExcelFileName = BasePage.readURLTestDataProp("MoneyMarket_TestData");

	String Scenario = "MC FutureDate(T+2) No change INT";
	...
	public void Create_DepositPlacement() throws Exception
	{
		...
		test.log(Status.INFO, "MultiCurrency - Early Maturity(2 days) FullWithdrawal(Future Date,No Change in Interest)"); 
	...
	{
		test=extent.createTest("Create Bulk Deposit Maturity(FullWithdrawal,No Change in INT)");
		 
		LoginPage.do_login(ExcelFileName,Scenario);
		BulkDepositMaturityPage.BulkDeposit_EarlyMaturity_Fullwithdrawal(ExcelFileName, Scenario,"","MultiCurrency");
